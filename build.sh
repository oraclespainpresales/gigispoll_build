...
echo Create Stack...
stackocid=`oci resource-manager stack create --compartment-id ${compartmentid} --terraform-version 0.12.x --config-source gigispoll_rm.zip --display-name ormgigispoll --variables '{"availability_domain":"'${availabilitydomainname}'","compartment_id":"'${compartmentid}'","shape":"VM.Standard.E2.1"}' --raw-output --query "data.id"`
...
echo ${publicip}
chmod 600 nopassphrase.key
echo Wait until Compute SSH connection is available...
ssh -i nopassphrase.key  -o "ConnectTimeout=5" -o "ConnectionAttempts=20" -o "StrictHostKeyChecking=no" opc@${publicip} 'exit'
echo ssh exit code is $?
...
echo Unzip VBCS app...
ssh -i nopassphrase.key -o "StrictHostKeyChecking=no" opc@${publicip} 'sudo unzip -qq /tmp/built-assets.zip -d /usr/share/nginx/html/gigispoll;sudo chown -R nginx:nginx /usr/share/nginx/html/gigispoll/'
echo Updating FN...
