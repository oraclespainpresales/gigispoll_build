...
echo Create Stack...
stackocid=`oci resource-manager stack create --compartment-id ${compartmentid} --terraform-version 0.12.x --config-source /tmp/build/gigispoll_rm.zip --display-name ormgigispoll --variables '{"compartment_id":"'${compartmentid}'","shape":"VM.Standard.E2.1"}' --raw-output --query "data.id"`
